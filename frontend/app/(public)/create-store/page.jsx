...
export default function CreateStore() {
  ...
  const fetchSellerStatus = async () => {
    try {
      ...
      if (["approved", "rejected", "pending"].includes(data.status)) {
        ...
        switch (data.status) {
          ...
          case "pending":
            setMessage(
              "Your store is pending approval, wait for admin to approve"
            );
            break;
          default:
            break;
    ...
    console.log("Seller Status:", status), message;
    setLoading(false);
  };
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      console.log("Form Data:", storeInfo);
      ...
      const { data } = await axios.post(`${apiUrl}/store/create`, formData, {
        headers: {
    ...
    <>
      {!alreadySubmitted ? (
        <div className="mx-6 min-h-[70vh] my-16">
          {/* Here onSubmitHandler return a promise and based on the state of the promise tost will show different messages */}
          <form
            onSubmit={(e) =>
